...
const router = new VueRouter({ // 配置对象
  ...
  /* 
  功能: 界面都有返回时自动停留在离开时的位置
  savedPosition: 传入的是离开时自动保存到对应的历史记录中的位置数据
  前提: 必须是点击浏览器返回/前进按钮 (back()/forword()/go(number))
  */
  scrollBehavior (to, from, savedPosition) {
    ...
    // 只有Search是, 其它都是停留在0,0的位置
    if (savedPosition && to.meta.isSavePosition) {
      return savedPosition
    } else {
      return { x: 0, y: 0} 
    }
